...
function tagValue(xml = "", tagName = "") { const n=tagName.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"); return cleanText(xml.match(new RegExp(`<${n}(?:\\s[\
^>]*)?>([\\s\\S]*?)<\\/${n}>`,`i`))?.[1]||""); }
...
function youtubeId(raw="") { for(const p of [/(?:youtube\.com\/watch\?[^\s"'<>]*v=|youtu\.be\/|youtube\.com\/(?:shorts|embed)\/)([A-Za-z0-9_-]{6,})/i,
/<yt:videoId[^>]*>([^<]+)<\/yt:videoId>/i]){const m=String(raw).match(p);if(m?.[1])return m[1].trim()}return"" }
export function parsePodcastItems(xml="", request) {
  return (String(xml||"").match(/<item\b[\s\S]*?<\/item>/gi)||[]).map(item=>{
    ...
    const title=tagValue(item,"title"), description=tagValue(item,"itunes:summary")||tagValue(item,"description"), link=normaliseUrl(tagValue(item,"li\
nk"),request);
    const slug=slugify((link.split('/').filter(Boolean).pop())||title);
    return {slug,title,description,teaser:firstSentence(description),episode_url:link||new URL(`/podcast/episodes/${slug}/`,request.url).toString(),transcript_url:normaliseUrl(attrValue(transcript,
"url"),request),audio_url:normaliseUrl(attrValue(enclosure,"url"),request),published_at:tagValue(item,"pubDate"),duration:tagValue(item,"itunes:durati\
on"),youtube_video_id:youtubeId(item),guid:tagValue(item,"guid")};
  }).filter(x=>x.title);
}
export async function fetchPodcastEpisodes(env, request, limit=3) {
  ...
  for(let i=0;i<attempts;i++){
    ...
    try{const r=await fetch(feedUrl,{headers:{Accept:"application/rss+xml, application/xml, text/xml"},signal:controller.signal});if(!r.ok)throw new Error(`\
HTTP ${r.status}`);const xml=await r.text();const episodes=parsePodcastItems(xml,request);if(!episodes.length)throw new Error('no items');return{feedUrl,
