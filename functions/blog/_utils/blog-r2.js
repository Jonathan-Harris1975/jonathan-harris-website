...
function youtubeEmbedMarkup(source = "") {
  ...
  return `<section class="card blog-video-embed" data-jh-blog-video="1"><h2>Watch the episode</h2><div class="responsive-media"><iframe src="https://w\
ww.youtube-nocookie.com/embed/${match[1]}" title="Video for this weekly AI briefing" loading="lazy" allow="accelerometer; autoplay; clipboard-write; e\
ncrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div></section>`;
}
function stripMarkup(value = "") {
  return String(value || "").replace(/<script\b[\s\S]*?<\/script\b[^>]*>/gi, " ").replace(/<style\b[\s\S]*?<\/style\b[^>]*>/gi, " ").replace(/<[^>]+>/g,
...
function blogShareCardParams(source = "") {
  ...
  const quote = source.match(/<blockquote\b[^>]*>([\s\S]*?)<\/blockquote>/i) ||
    source.match(/<p\b[^>]*class=(?:"[^"]*(?:standfirst|lead|summary)[^"]*"|'[^']*(?:standfirst|lead|summary)[^']*')[^>]*>([\s\S]*?)<\/p>/i);
...
<h2>Share this briefing</h2><nav class="share-links" aria-label="Share this briefing"><a href="https://twitter.com/intent/tweet?url=${encodedUrl}" tar\
get="_blank" rel="noopener noreferrer">Share on X</a><a href="https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}" target="_blank" rel="\
noopener noreferrer">LinkedIn</a><a href="https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}" target="_blank" rel="noopener noreferrer">Facebo\
ok</a><button type="button" data-copy-url="${canonical}">Copy link</button>${(() => { const card = blogShareCardParams(source); return `<a href="/api/\
blog/share-card?title=${encodeURIComponent(card.title)}&quote=${encodeURIComponent(card.quote)}" target="_blank" rel="noopener">Share graphic</a>`; })()}\
</nav>
<p class="jh-related-callout">Continue with the <a href="/topics/">AI topic guides</a>, <a href="/glossary/">plain-English glossary</a>, <a href="/ebo\
oks/">eBook catalogue</a>, or <a href="/podcast/">podcast archive</a>.</p>
</section>`;
}
