...
Release gate: fails when any routed HTML file in the repo is absent from the
workbook Pages sheet.

Excludes:
  - assets/partials/      (shared partials, not routed pages)
  - scripts/templates/    (template sources, not routed pages)
  - any path whose first segment starts with '.' (hidden dirs)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Fail when routed HTML files are absent from the workbook Pages sheet."
    )
    ...
    if errors:
        for e in errors:
            print(f"[FAIL] {e}")
        print(f"\nUngoverned route check failed: {len(errors)} unregistered route(s).")
        return 1

    print(f"[PASS] Ungoverned route check: all routed HTML files are governed in the workbook.")
    return 0
