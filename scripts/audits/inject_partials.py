...
CANONICAL_VIEWPORT_META = '<meta content="width=device-width, initial-scale=1, viewport-fit=cover" name="viewport"/>'
...
def ensure_viewport_head_block(text: str) -> tuple[str, bool]:
    """Normalise the governed mobile viewport meta tag in the document head."""
    if validate_viewport_head_block(text) is None:
        return text, False

    cleaned = _VIEWPORT_META_RE.sub("", text)
    charset_match = re.search(r'<meta\s+charset=["\'][^"\']+["\']\s*/?>', cleaned, re.IGNORECASE)
    if charset_match is not None:
        insert_at = charset_match.end()
        updated = cleaned[:insert_at] + "\n" + CANONICAL_VIEWPORT_META + cleaned[insert_at:]
        return updated, updated != text
...
def inject(dry_run: bool = False) -> int:
    ...
    for page in pages:
        ...
        updated_text, viewport_changed = ensure_viewport_head_block(updated_text)

        if (
            existing_header_block == header_partial
            and existing_footer_block == footer_partial
